...
void timer_Int_Init(void)
{
	...
	TIM_TimeBaseInitStructure.TIM_Period = TIM2_PERIOD; 	//(arr)
	TIM_TimeBaseInitStructure.TIM_Prescaler = TIM2_PRESCALER;   //(psc)  Tclk = 84Mhz  // Tout = ((arr+1)*(psc+1))/Tclk
...
{				 
	...
	TIM_TimeBaseStructure.TIM_Prescaler = PWM_PRESCALER;  //定时器分频
